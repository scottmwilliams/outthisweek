...
var tomatoesRequest = new XMLHttpRequest();
 tomatoesRequest.open("GET", 'http://api.rottentomatoes.com/api/public/v1.0/lists/dvds/new_releases.json?apikey='+apikeys.rottentomatoes+'&page_limit=20&page=1&country=us', true);
 tomatoesRequest.send(null);
 tomatoesRequest.onreadystatechange = function() {
      var tomatoesResponse = tomatoesRequest.responseText;
      var tomatoesRecent = JSON2.parse(tomatoesResponse);
      var i = 0;
 ...
 configRequest.open("GET", 'http://api.themoviedb.org/3/configuration?api_key='+apikeys.themoviedb, true);
 configRequest.send(null);
 console.log(configRequest.responseText);
 configRequest.onreadystatechange = function() {
        console.log(configRequest.responseText);
        fs.writeFile(path+'/'+'mdbConfig.json', JSON.stringify(configRequest.responseText, null, 4));
  };
}
